...
const SCENARIO_MODULES = [
  ...
  },
  {
    base: "D",
    code: "D",
    title: "Benefits beyond life cycle stage",
    desc: "Potential benefits or loads beyond the system boundary.",
    area: "area-d",
    tone: "tone-benefits",
    codeClass: "code-benefits"
  }
];
...
function getScenarioEmoji(name) {
  ...
  if (text.includes("shredd")) return "⚙️";
  if (text.includes("disposal")) return "🏁";
...
function renderScenarios(scenarios) {
  if (!scenarios.length) {
    ...
    dom.scenarioList.innerHTML = `<div class="empty-panel">No scenario details are available for this variant.</div>`;
  ...
  if (!impact) {
    ...
    dom.resultsLifecycleGrid.innerHTML = `
      <div class="empty-panel">
        No matching result was found in impacts.csv for this selected variant.
      </div>
    `;
    return;
  }
...
function createSingleTile(module, impact, toggleTarget = "") {
  ...
  return `
    <div class="result-tile ${module.area} ${module.tone} tile-appearing ${isToggle ? "toggle-tile" : ""}" ${isToggle ? `data-toggle-target="${toggleTarget}"` : ""}>
      ...
      ${isToggle ? `<div class="toggle-hint"> </div>` : ""}
    </div>
  `;
}
...
function buildModuleChartGroups(impact, selectedScenarios) {
  ...
  return groups.filter((group) => group.bars.length > 0);
}
...
function getLegendDotStyle(index) {
  if (index === 0) {
    return "background: linear-gradient(180deg, rgba(93, 140, 255, 0.96), rgba(122, 168, 255, 1));";
  }
  if (index === 1) {
    return "background: linear-gradient(180deg, rgba(207, 90, 64, 0.96), rgba(235, 125, 99, 1));";
  }
  return "background: linear-gradient(180deg, rgba(96, 200, 191, 0.96), rgba(131, 225, 214, 1));";
}
...
function initTheme() {
  const savedTheme = localStorage.getItem("slca-theme");
...
function toggleTheme() {
  ...
  localStorage.setItem("slca-theme", nextTheme);
}
